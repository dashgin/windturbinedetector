...
from backend.app.models import ELECTRIC_USAGE_PER_YEAR, Horizontal, Vertical, CO2Emission
...
def get_location(data):
    return max(data, key=(lambda x: x["efficiency"]))["location"]
...
def get_co2_emission(kwatt: float) -> float:
    ...
    return {
        "coal": f"{for_coal} kg/MJ",
        "gas": f"{for_gas} kg/MJ",
        "biomass": f"{for_biomass} kg/MJ",
        "hydro": f"{for_hydro} kg/MJ",
        "solar": f"{for_solar} kg/MJ",
        "geothermal": f"{for_geothermal} kg/MJ",
        "nuclear": f"{for_nuclear} kg/MJ",
        "wind_horizontal": f"{for_wind} kg/MJ",
    }
...
def get_result():
    _full_df = pd.read_csv("data_all.csv")

    grouped_by_location = _full_df.groupby("location")

    result = [
        {
            "location": location,
            "result": get_result_for_one_location(group),
        }
        for location, group in grouped_by_location
    ]
    return result
